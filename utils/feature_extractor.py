...
class FeatureExtractor(object):
    ...
    def parse_default_layers(self):
        """
        Assume that the model consists of convs and linear,
        ...
        try:
            num_convs = len(list(self.model.convs))
        except:
            Warning('No convs')
            num_convs = 0

        try:
            num_linear = len(list(self.model.linear))
        except:
            Warning('No linears')
            num_linear = 0

        for i in range(num_convs):
            layer_name = 'convs.{}'.format(i)
            layer_names.append(layer_name)

        for i in range(num_linear):
            layer_name = 'linear.{}'.format(i)
            layer_names.append(layer_name)
